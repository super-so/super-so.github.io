...
(() => {
  ...
  const mountGallery = (gallery) => {
    ...
    const originalStageStyles = stageProperties.map((property) => ({
      property,
      value: gallery.style.getPropertyValue(property),
      priority: gallery.style.getPropertyPriority(property)
    }));
    ...
    const desktopLayout = matchMedia("(min-width: 765px)");

    const restoreStageStyles = () => {
      originalStageStyles.forEach(({ property, value, priority }) => {
        if (value) {
          gallery.style.setProperty(property, value, priority);
        } else {
          gallery.style.removeProperty(property);
        }
      });
    };
    ...
    const syncStageBounds = () => {
      ...
      const content = gallery.closest(".super-content");
      const parent = gallery.parentElement;

      if (!desktopLayout.matches || !wrapper || !content || !parent) {
        restoreStageStyles();
        return;
      }
      ...
      gallery.style.setProperty(
        "--carousel-stage-margin-left",
        `${contentRect.left - parentRect.left}px`
      );
    };
